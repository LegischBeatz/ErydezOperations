...
export default function RunDetail() {
  ...
  return (
    <div data-testid="run-detail-page">
      <PageHeader breadcrumb={<Link to="/automations?tab=runs" className="hover:text-brand">Automations · Runs</Link>}
        title={r.automation} status={<StatusChip value={r.result} />}
        freshness={`Run at ${fmtDateTime(r.ts)} · Retries: ${r.retries}`} />
      <div className="grid grid-cols-1 gap-4 p-6 lg:grid-cols-2">
        <SectionCard title="Explanation" className="lg:col-span-2">
          <AutomationExplain
        ...
        <SectionCard title="Conditions evaluated">
        ...
        <SectionCard title="Result">
          {r.errors ? (
            <InlineAlert toneName="danger" title="Run failed" testId="run-error-alert">{r.errors}</InlineAlert>
          ) : (
            <InlineAlert toneName="ok" title="Run succeeded" testId="run-success-alert">All actions completed.</InlineAlert>
          )}
          <div className="mt-3">
            <FactList facts={[
              ["External response IDs", r.external_ids.length ? r.external_ids.join(", ") : "None"],
              ["Retries", r.retries],
              ["Resulting records", r.records.join("; ")],
            ]} />
          </div>
        </SectionCard>
        <SectionCard title="Raw payload (admin only)" className="lg:col-span-2">
          <button onClick={() => setRawOpen(!rawOpen)} className="flex items-center gap-1 text-sm font-medium text-brand hover:underline" data-testid="raw-payload-toggle">
            {rawOpen ? <ChevronDown size={14} /> : <ChevronRight size={14} />} {rawOpen ? "Hide" : "Show"} raw payload
          </button>
          {rawOpen && (

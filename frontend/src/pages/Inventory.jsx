...
export default function Inventory() {
  ...
  return (
    <div data-testid="inventory-page">
      <PageHeader title="Inventory" freshness="Available to promise = On hand − Committed − Quality hold · Projected = ATP + Confirmed inbound" />
      <div className="p-6">
        {isLoading ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-line text-left text-xs font-semibold text-inkmed">
                  {["Product", "SKU", "On hand", "Committed", "ATP", "Awaiting allocation", "Inbound (ETA)", "Oldest waiting order", "Reorder point", "Risk", "Recommendation"].map((h) => (
                ...
                {items?.map((i) => (
                  <tr key={i.sku} onClick={() => setParams({ sku: i.sku })} data-testid={`inventory-row-${i.sku}`}
                    ...
                    <td className="tnum px-3 text-xs">
                      {i.inbound_qty ? <>{i.inbound_qty} · {fmtDate(i.inbound_eta)} <span className={i.inbound_confidence === "Confirmed" ? "text-ok" : "text-warn"}>({i.inbound_confidence})</span></> : "—"}
                    </td>
            ...
            <>
              ...
              <div className="mt-4 space-y-4">
                <div className="rounded-md border border-line bg-subtle p-3">
                  <FactList facts={[
                    ["On hand", detail.on_hand], ["Committed to paid orders", detail.committed],
                    ["Quality hold", detail.quality_hold], ["Available to promise", detail.atp],
                    ["Confirmed inbound", detail.inbound_qty || 0], ["Projected available", detail.projected_available],
                    ["Sales velocity", `${detail.velocity_per_week} / week`],
                  ]} />
                </div>
                <div>
                  <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-inkmed">Waiting-order queue (allocation rule: oldest paid first)</p>
                  ...
                  )) : <EmptyState title="No waiting orders" />}
                </div>
                <div>
                  <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-inkmed">Inbound purchase orders</p>
                  {detail.inbound_pos?.length ? detail.inbound_pos.map((po) => (
                    <div key={po.id} className="rounded-md border border-line p-2.5 text-sm" data-testid="inbound-po">
                      ...
                      <p className="text-xs text-inkmed">{po.items} · {po.state}{po.eta ? ` · ETA ${fmtDate(po.eta)} (${po.eta_confidence})` : ""}</p>
                    </div>
                  )) : <p className="text-xs text-inkmed">No inbound purchase orders.</p>}
                </div>
                <InlineAlert toneName="info" testId="reorder-recommendation">
                  <span className="font-semibold">Reorder recommendation:</span> {detail.recommendation}. Assumptions: current velocity {detail.velocity_per_week}/week, supplier lead time per purchasing data. Recommendations are never presented as confirmed stock.
                </InlineAlert>
                <div>
                  <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-inkmed">Inventory events</p>
                  {detail.events.map((e) => (
